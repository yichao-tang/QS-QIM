function [MMT,H]=FrPCET_func(img,T,a)
...
%下面计算PCET矩
MMT(3,:)=0;
R=r.^a; %首先将r^2算出来，以便后面直接调用
MMT=complex(MMT);
for k=1:size(MMT,2)
    H{k}=exp(i*( 2*pi*MMT(1,k).*R + MMT(2,k).*theta)).*double(idx).*sqrt(r.^(a-2));
...
%imshow(uint8(Ir));
MMT(3,:)=MMT(3,:)*4/(pi*N^2)*a/2;%计算的PCET矩
MMT_real=abs(MMT);
