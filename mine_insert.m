function MMT=mine_insert(img,T,Mw,insert_palace)
...
for k=1:size(MMT,2)
    ...
    Ir=Ir+MMT(3,k)*H.*double(idx);
     if ismember(k,insert_palace)
        Iw=Iw+(Mw(k)/MMT(3,k)-1)*H.*MMT(3,k)*double(idx);
     end
end
% imshow(uint8(Ir));
Iw=abs(Iw)
 ...
 imshow(uint8(img));
a=max(max(img));
b=floor(max(max(abs(Ir))));
 img=img+max(max(img))*Iw./floor(max(max(abs(Ir))));
 imshow(uint8(img));
